...
def train(epoch, config, model, dataloaders, criterion, optimizer, scheduler, text_ori, ema_model):
    ...
    for batch_idx, (target_img, target_truth_label, target_pseudo_label, target_img_name) in enumerate(tqdm(target_loader)):
        ...
        # target_bce_loss = criterion['mae_loss'](target_output['logits'].sigmoid(), target_pseudo_label) # 目标域分类损失
        # target_cl_loss = criterion['target_cl_loss'](target_output['vision_features'], target_output['text_features'], target_label) # 源域对比损失
        target_bce_loss = torch.tensor(0.0).cuda()
        # target_cl_loss = torch.tensor(0.0).cuda()
        target_cl_loss = criterion['target_cl_loss'](target_output['au_vision_features'], target_output['au_text_features'], target_pseudo_label)
        
        loss = source_bce_loss + config.target_loss_weight*target_bce_loss + config.target_cl_loss_weight * target_cl_loss # 整体损失
        loss.backward()
        optimizer.step()
        ...
        if batch_idx == 0 or (batch_idx % (len_dataloader // 10) == 0):
            logger.info(f'Epoch: {epoch} | Batch: {batch_idx}/{len_dataloader} | loss: {loss_all.avg:.6f} | source_bce_loss: {loss_source_bce.avg} | target_bce_loss: {loss_target_bce.avg} | target_cl_loss: {loss_target_cl.avg}')
...
def val(epoch, config, model, dataloaders, criterion, text_ori):
    # 记录损失
    loss_all = AverageMeter()
    ...
    display_interval = len_dataloader // 5
    ...
    with torch.no_grad():
        for batch_idx, (images, labels, imag_name) in enumerate(tqdm(val_loader)):        
            ...
            output = model(images, text_ori) # forward pass
            # bce_loss = criterion['source_bce_loss'](output['logits'], labels) # 源域分类损失
            # cl_loss = criterion['source_cl_loss'](output['vision_features'], output['text_features'], labels)
            bce_loss = torch.tensor(0.0).cuda()
            cl_loss = torch.tensor(0.0).cuda()
            loss = bce_loss + config.source_cl_loss_weight * cl_loss# 整体损失
            loss_bce.update(bce_loss.data.item(), config.batch_size)
            loss_cl.update(cl_loss.data.item(), config.batch_size)
            loss_all.update(loss.data.item(), config.batch_size)
...
def pseudo_label(epoch, config, dataloaders, model, ema_model, text_ori):
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (images, labels, img_name) in enumerate(tqdm(target_train_loader)):
            ...
            for _ in range(config.f_pass):
                output = ema_model(images, text_ori)
        ...
        pseudo_labels = get_acc_pseudo_label(config.kappa_p, config.tau_p)
...
def uda_train(epoch, config, model, dataloaders, pseudo_labels, criterion, optimizer, scheduler, text_ori):
    # 记录损失
    loss_target_bce = AverageMeter()
    loss_target_cl = AverageMeter()
    loss_all = AverageMeter()
    ...
    statistics_list = None
    config.stop += 1 # early stop
    
    target_loader = dataloaders['target_loader']
    len_dataloader = len(target_loader)
    pseudo_labels = torch.tensor(pseudo_labels).float().cuda()
   
    for batch_idx, (target_img, target_label) in enumerate(tqdm(target_loader)):
        if torch.cuda.is_available():
            target_img, target_label = target_img.cuda(), target_label.float().cuda()
        
        p_label = pseudo_labels[batch_idx*target_img.shape[0]:(batch_idx+1)*target_img.shape[0]]
        
        optimizer.zero_grad()
        target_output = model(target_img, text_ori)
        logits = target_output['logits']
        
        target_bce_loss = criterion['target_bce_loss'](logits.sigmoid(), p_label) # 源域分类损失
        target_cl_loss = torch.tensor(0.0).cuda()
        
        loss = target_bce_loss + 0.1*target_cl_loss # 整体损失
        loss.backward()
        optimizer.step()
        scheduler.step()
        ...
        update_list = statistics(logits.sigmoid().detach(), target_label.detach(), 0.5)
        statistics_list = update_statistics_list(statistics_list, update_list)
...
def main(args):
    ...
    if config.train_or_test == 'train':
        ...
        # model(input, text_ori)
        # ema_m.module(input, text_ori)
        ...
        ema_val_output = val(9999, config, ema_m.module, dataloaders, criterion, text_ori)
        logger.info({'Epoch: [{:2d}/{:2d}], test_f1: {:.2f}, test_acc: {:.2f}, test_loss: {:.6f}, bce_loss: {:.6f}, cl_loss: {:.6f}'.format(9999, config.epochs, 100.*ema_val_output['mean_f1_score'], 100.*ema_val_output['mean_acc'], ema_val_output['loss'], ema_val_output['bce_loss'], ema_val_output['cl_loss'])})
        logger.info({'Val F1-score-list:'})
        ...
        for epoch in range(1, config.epochs + 1):
            ...
            pseudo_label_loader = pseudo_label(epoch, config, dataloaders, model, ema_m.module, text_ori)
            dataloaders['pseudo_label_loader'] = pseudo_label_loader
            
            logger.info('==> Training...')
            train_output = train(epoch, config, model, dataloaders, criterion, optimizer, scheduler, text_ori, ema_m)
            logger.info({'Epoch: [{:2d}/{:2d}], train_f1: {:.2f}, train_acc: {:.2f}, train_loss: {:.6f}, source_bce_loss: {:.6f}, bce_loss: {:.6f}, cl_loss: {:.6f}'.format(epoch, config.epochs, 100.*train_output['mean_f1_score'], 100.*train_output['mean_acc'], train_output['loss'], train_output['source_bce_loss'], train_output['target_bce_loss'], train_output['target_cl_loss'])})
            ...
            logger.info('==> Validation...')
            val_output = val(epoch, config, model, dataloaders, criterion, text_ori)
            logger.info({'Epoch: [{:2d}/{:2d}], test_f1: {:.2f}, test_acc: {:.2f}, test_loss: {:.6f}, bce_loss: {:.6f}, cl_loss: {:.6f}'.format(epoch, config.epochs, 100.*val_output['mean_f1_score'], 100.*val_output['mean_acc'], val_output['loss'], val_output['bce_loss'], val_output['cl_loss'])})
            logger.info({'Val F1-score-list:'})
            logger.info(dataset_info(val_output['f1_score_list']))
            logger.info({'Val Acc-list:'})
            logger.info(dataset_info(val_output['acc_list']))
            ...
            logger.info('==> EMA Validation...')
            ema_val_output = val(epoch, config, ema_m.module, dataloaders, criterion, text_ori)
            logger.info({'Epoch: [{:2d}/{:2d}], test_f1: {:.2f}, test_acc: {:.2f}, test_loss: {:.6f}, bce_loss: {:.6f}, cl_loss: {:.6f}'.format(epoch, config.epochs, 100.*ema_val_output['mean_f1_score'], 100.*ema_val_output['mean_acc'], ema_val_output['loss'], ema_val_output['bce_loss'], ema_val_output['cl_loss'])})
            logger.info({'Val F1-score-list:'})
            logger.info(dataset_info(ema_val_output['f1_score_list']))
            logger.info({'Val Acc-list:'})
            logger.info(dataset_info(ema_val_output['acc_list']))
