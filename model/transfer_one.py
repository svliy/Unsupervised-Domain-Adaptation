...
class TransferNet(nn.Module):
    def __init__(self, config) -> None:
        ...
        # 这个方式是最好的吗？
        self.multi_scale_combiner = MultiScaleCombiner(4 * self.vision_dim, self.vision_dim, config.drop_rate)
        ...
        # --- Graph Network ---
        self.gnn = GNN(self.vision_dim, num_classes=self.num_classes, neighbor_num=(self.num_classes // 2) + 1, metric='cosine')

        # --- Independent AU Classifiers ---
        # Create a separate linear classifier for each AU
        self.au_classifiers = nn.ModuleList([
            nn.Sequential(nn.Linear(self.vision_dim, self.vision_dim // 2),
                          nn.ReLU(inplace=True),
    ...
    def forward(self, image, text=None, labels=None):
        ...
        au_pooled_features = au_features.reshape(B, self.config.class_num, self.vision_dim, -1) # [B, 10, 256, 49]
